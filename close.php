<?php
  ...
  session_start();
    ...
    unset($_SESSION["tabList"][$_POST["tab"]]);

    if($_SESSION["tab"] == $_POST["tab"]){
      unset($_SESSION["tab"]);
    }
    session_start();
    include("array.inc");
...
//感情タブ
    if(isset($_SESSION["tabList"])){
      ...
      foreach($_SESSION["tabList"] as $key => $value){
        ...
        echo '<img onclick="closeTab(\'' . $key . '\')" style="vertical-align:top;margin:8px 11px 0 -26px;" src="img/close.png" onmouseover="this.src=\'img/close_on.png\';" onmouseout="this.src=\'img/close.png\';">';
      }
    }
    echo '<div id="qaKanjoTab" style="display: none; position: absolute; top: 42px; left: 315px; padding: 10px; width: 180px; border: 1px solid rgb(187, 187, 187); border-radius: 8px; box-shadow: rgba(0, 0, 0, 0.298039) 0px 0px 14px 1px; z-index: 3; background-color: rgb(255, 255, 255);">みんなが評価した感情別に<br>検索できるよ！</div>';
